...
This project applies the permanent HyperFrames Channel system from the repository root `DESIGN.md`
over the broadside frame scale: `#0B0F14` canvas, `#101C29` surfaces, `#F5F7FA` ink, `#AAB7C4`
muted labels, `#2A3A4A` quiet rules, `#59A8FF` request blue, `#71D7B2` response mint, and
`#F6B450` warning amber. Use Barlow for display and IBM Plex Mono for addresses, table rows, and
packet payloads. Diagram objects stay sharp and rectangular; routers are compact circular/diamond
junctions with visible ports; packets are compact rectangles. One active signal color carries each
beat, with blue outbound and mint return traffic as the explicit exchange exception. No gradients,
generic card grids, or decorative connectors without real endpoints.
