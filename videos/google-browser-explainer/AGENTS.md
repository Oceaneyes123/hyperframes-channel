...
2. Generate the final continuous narration track first.
3. Derive one canonical scene timing table from that track; each scene maps to its matching narration beat.
4. Build frames against those locked timings and mount the narration as one audio track starting at `0`.
...
Do not invent an incompatible per-line audio metadata shape when the assembler expects a different contract. Avoid broad HTML/skill dumps: inspect only the failing selector or relevant section.
